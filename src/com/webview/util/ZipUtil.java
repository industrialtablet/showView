...
public final class ZipUtil {
	 ...
	 * @throws Exception
	 */
	public static void zipFolder(String srcFilePath, String zipFilePath,List<String> islog)throws Exception {
		//创建Zip包
		logs=null;
		logs=islog;
//		String zipFilename ="/"+new SimpleDateFormat("yyyyMMddHHmmss").format(new Date())+".zip";
		String zipFilename="/log.zip";
		java.util.zip.ZipOutputStream outZip = new java.util.zip.ZipOutputStream(new java.io.FileOutputStream(zipFilePath+zipFilename));
		//打开要输出的文件
	...
	private static void zipFiles(String folderPath, String filePath, java.util.zip.ZipOutputStream zipOut)throws Exception{
		...
		if (file.isFile()) {
			for (int i = 0; i < logs.size(); i++) {
				if(file.getName().equalsIgnoreCase(logs.get(i))){
					Log.i("is ok logs list:", logs.get(i));
