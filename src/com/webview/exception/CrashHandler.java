...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				Toast.makeText(mContext, "数据获取异常，即将重启程序", Toast.LENGTH_SHORT).show();
			}
		}.start();
		//保存日志文件     
	...
	private String getFilePath() {
		...
		if (isSDCardExist && isRootDirExist) {
			...
			// ChatApp.getInstance().getFilesDir()返回的路劲为/data/data/PACKAGE_NAME/files，其中的包就是我们建立的主Activity�?��的包
			file_dir = ShowView.getInstance().getFilesDir().getAbsolutePath() + "/crashlog/";
		}
		return file_dir;
	}
	/**
